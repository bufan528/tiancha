...
[Investment-research retention block. When the conversation involves research, scoring, due diligence, or data analysis, preserve verbatim and do NOT collapse or paraphrase away:]
- **Key data points**: every quantitative figure already established (market size / TAM, CAGR, valuation, revenue, multiples, policy dates, thresholds), with units and as-of dates
- **Conclusions**: each interim or final conclusion reached, and its current confidence/consensus status
- **Sources**: the provenance of each claim (source name / document / URL / tool output id / date) so later steps can cite and re-check it
- **Scoring basis**: dimension scores, weights, anchors applied, and the rationale for the current total score / grade and whether it meets the enter-pool threshold
- **Open questions / contradictions**: unresolved gaps, conflicting sources, or assumptions still unverified
- If the conversation is not research/scoring-related, write "(not applicable)".

Keep each section concise. Preserve exact file paths, function names, and error messages. Never drop numbers, citations, or scoring rationale to save space—those are load-bearing for downstream work.`;
...
- PRESERVE the "Key Evidence & Scoring" block: keep prior data points, conclusions, sources, and scoring basis; append new evidence and update scores/grades as they change; never delete a citation or a score unless it was explicitly superseded, and note the supersession
...
## Key Evidence & Scoring
- [Preserve prior data points / conclusions / sources / scoring basis; append new evidence; update scores and note supersessions; "(not applicable)" if no research content]

Keep each section concise. Preserve exact file paths, function names, and error messages. Never drop numbers, citations, or scoring rationale to save space.`;
...
Be concise. Focus on what's needed to understand the kept suffix. When research/scoring is involved, retain any key numbers, conclusions, source citations, and scoring rationale that the suffix depends on.`;
